...
function guardarReparacionEnLocalStorage(reparacion) {
    let reparaciones = JSON.parse(localStorage.getItem('reparaciones')) || [];

    reparacion.costoTotal = reparacion.costo + reparacion.repuestos.reduce((acc, repuesto) => acc + repuesto.valor, 0);
    reparaciones.push(reparacion);
    
    localStorage.setItem('reparaciones', JSON.stringify(reparaciones));
}

function cargarReparacionesDesdeLocalStorage() {
    let reparaciones = JSON.parse(localStorage.getItem('reparaciones')) || [];
...
function realizarAutorizacion(telefono, sucursal, diagnostico, costo, repuesto, nombreCliente, abono, autorizacionEscrita) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            guardarReparacionEnLocalStorage(reparacion);
            agregarReparacionATabla(reparacion);
            Swal.fire({
...
function agregarReparacionATabla() {
    ...
    // Recorrer cada reparación y agregarla a la tabla
    reparaciones.forEach(reparacion => {
        ...
        const row = `
            <tr>
                ...
                <td>${reparacion.telefono.estado}</td>
                ...
                <td>
                <button class="btn btn-success btn-sm" onclick="finalizarReparacion('${reparacion.telefono.numeroSerie}')"
                    ${reparacion.telefono.estado === 'Reparado' ? 'disabled' : ''}>Reparar</button>
            </td>
            </tr>
        `;
        tabla.innerHTML += row;
...
function finalizarReparacion(numeroSerie) {
    ...
    reparaciones = reparaciones.map(reparacion => {
        if (reparacion.telefono.numeroSerie === numeroSerie) {
            reparacion.telefono.estado = 'Reparado';
        }
        return reparacion;
    });
    localStorage.setItem('reparaciones', JSON.stringify(reparaciones));
...
document.addEventListener('DOMContentLoaded', function () {
    cargarReparacionesDesdeLocalStorage();
});
