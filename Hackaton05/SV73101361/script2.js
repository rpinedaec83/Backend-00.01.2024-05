...
$(document).ready(function () {
    ...
    $('#phoneForm').on('submit', function (event) {
        ...
        const autorizacion = $('#autorizacion').val().toLowerCase() === 'sí';
        ...
        const row = `<tr>
            ...
            <td>${modelo}</td>
            <td>${diagnostico}</td>
            <td>${autorizacion ? 'Sí' : 'No'}</td>
            <td>${abono}</td>
            <td class="estado">${ticket.estado}</td>
            <td><button class="action-button" onclick="cambiarEstado(this)">Cambiar Estado</button></td>
        </tr>`;
