...
export const getAllItems = async (
  ...
  try {
    ...
    res.end(
      JSON.stringify({
        error: "Error al obtener los items de la lista de compras",
      })
    );
  }
};
...
export const getItemById = async (
  ...
  try {
    ...
    res.end(
      JSON.stringify({
        error: "Error al obtener el item de la lista de compras",
      })
    );
  }
};
...
export const createItem = async ( req: IncomingMessage , res: ServerResponse): Promise<void> => {
    ...
    req.on('end', async () => {
        try {
            ...
            res.writeHead(500, {'Content-Type' : 'application/json'})
            res.end(JSON.stringify({error: 'Error al crear el item de compra'}))
        }
    });
};
...
export const updateItem = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    req.on('end', async() => {
        try {
            ...
            const [updated] = await ShoppingList.update(updatedData,{ where : {id}
            ...
            res.writeHead(500,{'Content-Type': 'application/json'});
            res.end(JSON.stringify({error: 'Error al actualizar la lista de compras'}))
        }
    })
};
